...
TOutTextParams = packed record
  DispNo,         // Номер дисплея (0x00 - по умолчанию, 0x01 - текстовый, 0x02 - сегментный, 0x03 - резерв)
  Font,           // Номер шрифта (от 0 до 7)
  Align,          // Выравнивание (0 влево, 1 - вправо, 2 - по центру)
  Opt,            // Опции показа (0x80 - сохранить, 0x01 - пауза для бегущего текста)
  Speed,          // Скорость движения тескста (от 0 до 5) Бит 7 (0x80) задает адаптивную скорость
  Color,          // Цвет (от 1 до 7), 1 - красный, 2 - зеленый, 3 - синий
  Effect,         // Эффект: 0 - без эффекта, 1 - пауза для бегущего текста, 2 - мигание
  Iterat,         // Кол-во повторов эффекта
  Interval :byte; // Временной интервал эффекта
end;
...
function OpenPort:boolean;
begin
  if WorkPort.PortName <> WorkPortName then
  begin
  ...
  if Result then
  begin
    if WorkPort.LANConnection then
      WaitTimeOut := 1000
    else
      WaitTimeOut := 300;
  end;
...
begin
  if (DataSize>RecBuffSizeByte) or (DataSize<0) then
  begin
  ...
  SB.DataLen:= DataSize;
  if (DataSize>0) then
    Move(Data^, SB.Data, DataSize);
  if not ExchangeDataPacket(@SB, @RB, TimeOut) then
  begin
  ...
  Result:= RB.Status;
  if (DB<>Nil) then
    Move(RB, DB^, RB.DataLen+sizeof(TLevel2Head));
end;
...
begin
  ...
  OutSize:= D.DataLen;
  if (OutSize>0) and (OutBuff<>Nil) then
    Move(D.Data, OutBuff^, OutSize);
end;
